...
const ScopeButton = styled.button`
  ...
  &:hover {
    background-color: ${(props) =>
      props.selected ? props.theme.secondary : "#ccc"};
  }
`;
...
const ScopeDescription = ({ scope }) => {
  ...
  else return <Description>다이어리를 비공개합니다.</Description>;
};
    ...
    isOn && (
      <ModalBackdrop>
        <Container>
          <Header>
            다이어리 옵션
            <CloseBtn onClick={handleClickBtn}>
              <CloseSvg />
            </CloseBtn>
          </Header>
          <StampContainer>
          ...
          <Footer>
            <CompleteBtn onClick={handleCompleteBtn}>완료</CompleteBtn>
          </Footer>
        </Container>
      </ModalBackdrop>
    )
